...
FrogPilotVisualsPanel::FrogPilotVisualsPanel(SettingsWindow *parent) : FrogPilotListWidget(parent) {
  ...
  for (const auto &[param, title, desc, icon] : visualToggles) {
    ...
    if (param == "CameraView") {
      ...
      std::vector<QString> themeOptions{tr("Stock"), tr("Frog"), tr("Tesla"), tr("blood")};
      ...
      std::map<int, QString> steeringWheelLabels = {{0, "Stock"}, {1, "Lexus"}, {2, "Toyota"}, {3, "Frog"}, {4, "Rocket"}, {5, "Hyundai"}, {6, "blood"}};
