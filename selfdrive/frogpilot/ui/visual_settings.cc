...
FrogPilotVisualsPanel::FrogPilotVisualsPanel(SettingsWindow *parent) : ListWidget(parent) {
  ...
  for (const auto &[param, title, desc, icon] : visualToggles) {
    ...
    if (param == "CustomTheme") {
      ...
      toggle = new ParamValueControl(param, title, desc, icon, 0, 16, {{0, "Stock"}, {1, "Lime"}, {2, "Kirby"}, {3, "Blaze"}, {4, "Dusk"}, {5, "Hs&Hrs"}, {6, "Lust"}, {7, "Comic"}, {8, "Dawn"}, {9, "Freeze"}, {10, "Night"}, {11, "Gold"}, {12, "Jade"}, {13, "Baja"}, {14, "Torch"}, {15, "Blood"}, {16, "Bumble"}}, this);
