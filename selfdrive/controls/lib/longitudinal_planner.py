...
class LongitudinalPlanner:
  ...
  def check_conditions(self, sm, v_ego, v_lead):
    ...
    stop_light_detected = self.stop_sign_and_light(carstate, lead, lead_distance, modeldata, radarstate, v_ego, v_lead) and (self.stop_lights or self.curves) and not standstill
    ...
    self.curve = self.road_curvature(lead, modeldata, v_ego) and self.curves and not standstill and not stop_light_detected
    if self.curve:
      self.status_value = 8
      return True
