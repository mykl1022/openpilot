...
void AnnotatedCameraWidget::drawLaneLines(QPainter &painter, const UIState *s) {
  ...
  if (sm["controlsState"].getControlsState().getExperimentalMode() || frogColors) {
    ...
    for (int i = 0; i < max_len; ++i) {
      ...
      if (frogColors && std::abs(acceleration[i]) < 0.2) {
        ...
        float lightness = util::map_val(lin_grad_point, 1.0f, 1.0f, 0.0f, 1.0f); // Adjust the range and values as needed
        // Control alpha for transparency
        float alpha = util::map_val(lin_grad_point, 0.2f, 0.75f, 0.8f, 0.2f); // Adjust the range and values as needed
        bg.setColorAt(lin_grad_point, QColor::fromHslF(path_hue, 1.0, lightness, alpha));
  ...
  if (developerUI && speedCheck && isNotTurning && is_cruise_set) {
    ...
    const auto setGradientColors = [](QLinearGradient& gradient, const float laneWidth, const float minLaneWidth, const float maxLaneWidth, const bool blindspot) {
      ...
      gradient.setColorAt(1.0, QColor::fromHslF(hue / 360., 1.0, 0.20, 0.2));
    };
