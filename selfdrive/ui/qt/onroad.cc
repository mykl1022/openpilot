...
void AnnotatedCameraWidget::drawLaneLines(QPainter &painter, const UIState *s) {
  ...
  for (int i = 0; i < std::size(scene.road_edge_vertices); ++i) {
    if (frogColors) {
      painter.setBrush(QColor(70, 0, 255, 242));
    } else {
...
if (sm["controlsState"].getControlsState().getExperimentalMode() || frogColors) {
    ...
    for (int i = 0; i < max_len; ++i) {
        ...
        path_hue = fmax(fmin(path_hue, 120), 260);  // Ensure hue is within the valid range
    ...
    bg.setColorAt(0.0, QColor::fromHslF(80 / 360., 1.00, 0.18, 0.80)); // Start with purple
