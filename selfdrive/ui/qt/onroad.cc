...
void AnnotatedCameraWidget::drawLaneLines(QPainter &painter, const UIState *s) {
  ...
  for (int i = 0; i < std::size(scene.road_edge_vertices); ++i) {
    if (frogColors) {
      painter.setBrush(QColor(255, 0, 0, 242));
    } else {
  ...
  if (sm["controlsState"].getControlsState().getExperimentalMode() || frogColors) {
    ...
    bg.setColorAt(0.5, QColor::fromHslF(0 / 360., 1.0, 0.75, 0.35));
    bg.setColorAt(1.0, QColor::fromHslF(0 / 360., 1.0, 0.75, 0.1));
  }
  ...
  if (alwaysOnLateral) { // Pink & white
    pe.setColorAt(0.0, QColor::fromHslF(0 / 360., 1.0, 0.5, 1.0)); // Start with red
    pe.setColorAt(0.5, QColor::fromHslF(0 / 360., 1.0, 0.0, 1.0)); // Transition to black (full saturation and lightness)
    pe.setColorAt(1.0, QColor::fromHslF(0 / 360., 1.0, 0.0, 1.0)); // Stay at black
  } else if (conditionalStatus == 1) {
    pe.setColorAt(0.0, QColor::fromHslF(51 / 360., 1.0, 0.5, 1.0));
    pe.setColorAt(0.5, QColor::fromHslF(51 / 360., 1.0, 0.5, 1.0));
    pe.setColorAt(1.0, QColor::fromHslF(0 / 360., 1.0, 0.5, 1.0));
  } else if (experimentalMode) {
    pe.setColorAt(0.0, QColor::fromHslF(0 / 360., 1.0, 0.5, 1.0));
    pe.setColorAt(0.5, QColor::fromHslF(230 / 360., 1.0, 0.5, 1.0));
    pe.setColorAt(1.0, QColor::fromHslF(51 / 360., 1.0, 0.5, 1.0));
    ...
    pe.setColorAt(0.0, QColor::fromHslF(0 / 360., 1.0, 0.5, 0.6));
    pe.setColorAt(0.5, QColor::fromHslF(0 / 360., 1.0, 0.0, 1.0));
  ...
  if ((blindSpotLeft || blindSpotRight) && speedCheck && isNotTurning && is_cruise_set) {
    bs.setColorAt(0.0, QColor::fromHslF(20 / 360., 0.5, 0.50, 0.6));
    bs.setColorAt(0.5, QColor::fromHslF(20 / 360., 0.5, 0.50, 0.4));
    bs.setColorAt(1.0, QColor::fromHslF(20 / 360., 0.5, 0.50, 0.2));
  }
  ...
  if (developerUI && speedCheck && isNotTurning && is_cruise_set) {
    ...
    const auto setGradientColors = [](QLinearGradient& gradient, const float laneWidth, const float minLaneWidth, const float maxLaneWidth, const bool blindspot) {
      ...
      if ((laneWidth < minLaneWidth) || blindspot) {
        ...
        hue = (0 * (laneWidth - minLaneWidth)) / (maxLaneWidth - minLaneWidth);
      }
