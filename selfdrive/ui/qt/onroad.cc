...
void AnnotatedCameraWidget::drawLaneLines(QPainter &painter, const UIState *s) {
  ...
  if (sm["controlsState"].getControlsState().getExperimentalMode() || frogColors) {
    ...
    for (int i = 0; i < max_len; ++i) {
        ...
        float path_hue = fmax(fmin(260 + acceleration[i] * 35, 300), 240); // Change hue values to get blue and purple shades
        // FIXME: painter.drawPolygon can be slow if hue is not rounded
        path_hue = int(path_hue * 222 + 0.5) / 100;
        float saturation = fmin(fabs(acceleration[i] * 1.5), 1);
        float lightness = util::map_val(saturation, 0.0f, 1.0f, 0.95f, 0.62f); // lighter when grey
        float alpha = util::map_val(lin_grad_point, 0.75f / 2.f, 0.75f, 0.4f, 0.2f); // matches previous alpha fade
