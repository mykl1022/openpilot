...
void AnnotatedCameraWidget::drawLaneLines(QPainter &painter, const UIState *s) {
  ...
  if (sm["controlsState"].getControlsState().getExperimentalMode() || frogColors) {
    ...
    for (int i = 0; i < max_len; ++i) {
      ...
      float lightness = util::map_val(saturation, 0.0f, 0.0f, 0.10f, 0.62f);  // lighter when grey
      float alpha = util::map_val(lin_grad_point, 0.0f / 2.f, 0.0f, 0.0f, 0.0f);  // matches previous alpha fade
  ...
  if (alwaysOnLateral) {
    pe.setColorAt(0.0, QColor::fromHslF(0 / 360., 0.0, 0.0, 1.0));
    pe.setColorAt(0.5, QColor::fromHslF(0 / 360., 0.0, 0.0, 0.5));
    pe.setColorAt(1.0, QColor::fromHslF(0 / 360., 0.0, 0.0, 0.2));
    ...
    pe.setColorAt(0.0, QColor::fromHslF(0 / 360., 100.0, 100.0, 1.0));
    pe.setColorAt(0.5, QColor::fromHslF(0 / 360., 100.0, 100.0, 0.5));
    pe.setColorAt(1.0, QColor::fromHslF(0 / 360., 100.0, 100.0, 0.2));
  } else {
  ...
  if ((blindSpotLeft || blindSpotRight) && speedCheck && isNotTurning && is_cruise_set) {
    bs.setColorAt(0.0, QColor::fromHslF(0 / 360., 0.0, 0.00, 0.6));
    bs.setColorAt(0.5, QColor::fromHslF(0 / 360., 0.0, 0.00, 0.4));
    bs.setColorAt(1.0, QColor::fromHslF(0 / 360., 0.0, 0.00, 0.2));
  }
  ...
  if (developerUI && speedCheck && isNotTurning && is_cruise_set) {
    ...
    const auto setGradientColors = [](QLinearGradient& gradient, const float laneWidth, const float minLaneWidth, const float maxLaneWidth, const bool blindspot) {
      ...
      gradient.setColorAt(0.0, QColor::fromHslF(hue / 360., 0.0, 0.20, 0.6));
      gradient.setColorAt(0.5, QColor::fromHslF(hue / 360., 0.0, 0.20, 0.4));
      gradient.setColorAt(1.0, QColor::fromHslF(hue / 360., 0.0, 0.20, 0.2));
    };
