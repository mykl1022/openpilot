...
if (sm["controlsState"].getControlsState().getExperimentalMode() || frogColors) {
    ...
    for (int i = 0; i < max_len; ++i) {
        ...
        float hue = 60 + (acceleration[i] * 180); // Adjust the multiplier as needed
        if (hue < 0) {
    hue += 360;
} else if (hue > 360) {
    hue -= 360;
        ...
        float lightness = 0.5; // Moderate lightness
        float alpha = 1.0; // Full alpha
...
bg.setColorAt(0.0, QColor(148, 0, 211));
bg.setColorAt(0.2, QColor(75, 0, 130));  
bg.setColorAt(0.4, QColor(0, 0, 255));
bg.setColorAt(0.6, QColor(0, 255, 0));
bg.setColorAt(0.8, QColor(255, 255, 0));
bg.setColorAt(1.0, QColor(255, 127, 0));
  }
