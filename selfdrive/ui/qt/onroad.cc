...
void AnnotatedCameraWidget::drawLaneLines(QPainter &painter, const UIState *s) {
  ...
  if (sm["controlsState"].getControlsState().getExperimentalMode() || frogColors) {
    ...
    for (int i = 0; i < max_len; ++i) {
      ...
      float path_hue = fmax(fmin(60 + acceleration[i] * 35, 120), 1);
      // FIXME: painter.drawPolygon can be slow if hue is not rounded
      path_hue = int(path_hue * 100 + 0.5) / 100;

      float saturation = fmin(fabs(acceleration[i] * 1.5), 1);
      float lightness = util::map_val(saturation, 0.1f, 0.1f, 0.10f, 0.62f);  // lighter when grey
      float alpha = util::map_val(lin_grad_point, 0.2f / 2.f, 0.2f, 0.2f, 0.6f);  // matches previous alpha fade
